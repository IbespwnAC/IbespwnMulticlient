namespace ACMulticlient
{
  ...
  public class frmMain : Form
  {
    ...
    private const string DEF_STRING = "********";
    ...
    {
      UserEntry entry = getUserentry(cboUsername.Text);
    ...
    private void txtAc_KeyDown(object sender, KeyEventArgs e)
    {
      ...
      {
        if (!string.Empty.Equals(txtAc.Text) || ((entry != null) && (!entry.isSecretNull())))
        {
    ...
    private void txtAc_LostFocus(object sender, EventArgs e)
    {
      UserEntry entry = this.getUserentry(this.cboUsername.Text);
      save_credentials(ref entry);
    }
    ...
    private void cboUsername_KeyDown(object sender, KeyEventArgs e)
    {
      ...
      {
        UserEntry entry = this.getUserentry(this.cboUsername.Text);
        if ((entry != null) && (!entry.isSecretNull()))
        {
          this.btnLaunch_Click(null, EventArgs.Empty);
        }
        else
    ...
    private void InitializeComponent()
    {
      ...
      if (vers.Length - 2 <= 0)
      {
        this.Text = "AC Multi Client {Unable to detect version}";
      }
      else
      {
        this.Text = "AC Multi Client v" + vers.Substring(0, vers.Length - 2);
      }
    ...
    {
      if (entry.Equals(DEF_STRING))
      {
        return;
      }

      entry.setSecret(ref this._txtAc, DEF_STRING);
    }
    ...
    public void relaunchac(UserEntry useraccount, string acpath)
    {
      try
      {
        ...
        try
        {
          if (!useraccount.isSecretNull())
          {
            str2 = useraccount.getSecret();
          }
        }
        catch (SecurityException exception1)
        {
    ...
    internal virtual TextBox txtAc
    {
      ...
      set
      {
        ...
        if (this._txtAc != null)
        {
          this._txtAc.LostFocus -= this.txtAc_LostFocus;
